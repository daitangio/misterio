...
# set -v
cd $(dirname $0)
for role_env in ecs/$AWS_PROJECT_NAME/*.env ; do
    ...
    if [ "$#" == "1" -a "$1" == "apply" ] ; then    
        ( cd $role_dir ; 
            ( ecs-cli compose  -f docker-compose.yml up \
                 --create-log-groups --cluster-config $AWS_PROJECT_NAME --ecs-profile $AWS_PROJECT_NAME-profile
            ) || (echo FAILED $role on $HOSTNAME ))
    else
